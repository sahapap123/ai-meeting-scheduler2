...
function extractEventData(rawText: string) {
  ...
  // 🔥 ความฉลาดขั้นสุด: เช็คว่าเวลาผ่านไปแล้วหรือยัง? 🔥
  if (targetDate.getDate() === now.getDate() && hour <= now.getHours()) {
    if (hour < 12 && (hour + 12) > now.getHours()) {
      // เช่น พิมพ์ 10 โมง (ตอนบ่าย) -> เปลี่ยนเป็น 4 ทุ่ม (22:00) ให้
      hour += 12; 
    } else {
      // ถ้าเปลี่ยนเป็นกลางคืนแล้วก็ยังเลยเวลาอยู่ดี (เช่น พิมพ์บ่าย 2 ตอนบ่าย 3) -> ปัดไปพรุ่งนี้เลย!
      targetDate.setDate(targetDate.getDate() + 1);
    }
  }

  // --- 3. Big Cleaning: โยน "วันนี้" ลงถังขยะด้วย! ---
  // เพิ่มคำว่า วันนี้, พรุ่งนี้, มะรืน ลงไปด้วย เผื่อมันหลงเหลือในประโยค
  const junkWords = /เย็น|เช้า|บ่าย|ค่ำ|สาย|ดึก|โมง|นาฬิกา|น\.|เวลา|เดือนนี้|ตอน|ช่วง|ประมาณ|วันนี้|พรุ่งนี้|มะรืน/g;
